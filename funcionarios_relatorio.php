...
function Header()
  {
    ...
    $this->Cell(0, 15, utf8_decode("FUNCIONÁRIOS"), 0, 1, 'C');
...
$vRelatorio->Cell(160, 20, 'nome', 0, 0, "L", 1);
$vRelatorio->Cell(140, 20, 'data_nasc', 0, 0, "L", 1);
$vRelatorio->Cell(100, 20, 'telefone', 0, 0, "C", 1);
$vRelatorio->Cell(120, 20, 'cpf', 0, 0, "C", 1);
$vRelatorio->Cell(100, 20, 'rg', 0, 0, "C", 1);
$vRelatorio->Cell(170, 20, 'email', 0, 1, "C", 1);
...
while($vTabela=mysqli_fetch_array($vExecucao)) 
     {
      ...
      $vData_nasc=date('d/m/Y', strtotime($vTabela['data_nasc']));   
      $vTelefone=utf8_encode($vTabela['telefone']);   
      $vCpf=utf8_encode($vTabela['cpf']); 
      $vRg=utf8_encode($vTabela['rg']);  
      $vEmail=utf8_encode($vTabela['email']); 
      $vLogradouro=utf8_encode($vTabela['logradouro']);   
      $vNumero=utf8_encode($vTabela['numero']);
      $vBairro=utf8_encode($vTabela['bairro']); 
      $vComplemento=utf8_encode($vTabela['complemento']); 
      $vCidade=utf8_encode($vTabela['cidade']);   
      $vCep=utf8_encode($vTabela['cep']); 
      $vUf=utf8_encode($vTabela['uf']); 
      $vTipo=utf8_encode($vTabela['tipo']);	  
      //Formata a cor do texto
      $vRelatorio->SetTextColor(0,0,0);
      //Publica os valores dos campos do registro atual
      $vRelatorio->Cell(160, 35, $vNome, 0, 0, "L");
      $vRelatorio->Cell(140, 35, $vData_nasc, 0, 0, "L");
      $vRelatorio->Cell(100, 35, $vTelefone, 0, 0, "L");
      $vRelatorio->Cell(120, 35, $vCpf, 0, 0, "L");
	  $vRelatorio->Cell(100, 35, $vRg, 0, 0, "L");
	  $vRelatorio->Cell(170, 35, $vEmail, 0, 1, "L");
