...
$vSql='SELECT '.
      'caixa.id, '.
      'caixa.data, '.
      'historicos.descricao as "historico", '.
      'caixa.receita, '.
      'caixa.despesa '.
  ...
  <table id="vTabela" class="table table-striped table-hover">
    ...
    <thead>
      ...
      <tr>
		...
		<th title="Classificar" style="text-align:left; cursor:pointer" onclick="Ordenar(0)">Histórico</th>
		<th title="Classificar" style="text-align:left; cursor:pointer" onclick="Ordenar(0)">Receita</th>
		<th title="Classificar" style="text-align:left; cursor:pointer" onclick="Ordenar(0)">Despesa</th>
...
while($vTabela=mysqli_fetch_array($vExecucao)) 
     {
      $vData=utf8_encode($vTabela['data']);
	  $vHistorico=utf8_encode($vTabela['historico']);
	  $vReceita=utf8_encode($vTabela['receita']);
	  $vDespesa=utf8_encode($vTabela['despesa']);

      echo '<tr>
              <td style="text-align:left; vertical-align:middle;"> '. date('d/m/Y', strtotime($vTabela['data'])).' </td>
			  <td style="text-align:left; vertical-align:middle;"> '.$vHistorico.'</td>
			  <td style="text-align:left; vertical-align:middle;"> '. number_format($vTabela['receita'], 2, ',', '.') .' </td>
			  <td style="text-align:left; vertical-align:middle;"> '. number_format($vTabela['despesa'], 2, ',', '.') .' </td>';
